...
namespace DGtal
{
  ...
  */
  ...
  public:
    ...
    static
    Scalar mu0InterpolatedU
    ( const RealPoint& a, const RealPoint& b, const RealPoint& c, const RealPoint& d,
      const RealVector& ua, const RealVector& ub, const RealVector& uc, const RealVector& ud)
    {
      return (1/36.0) * ( ( 4 * ua + 2 * ub + 2 * uc +     ud ).dot( (b - a).crossProduct( c - a ) )
                        + ( 2 * ua + 4 * ub +     uc + 2 * ud ).dot( (b - a).crossProduct( d - b ) )
                        + ( 2 * ua +     ub + 4 * uc + 2 * ud ).dot( (d - c).crossProduct( c - a ) )
                        + (     ua + 2 * ub + 2 * uc + 4 * ud ).dot( (d - c).crossProduct( d - b ) ) );
    }
    ...
    /// @return the mu0-measure of the given polygonal face, i.e. its area.
    static
    Scalar mu0InterpolatedU( const RealPoints& pts, const RealVectors& u, int q,
			     bool unit_u = false )
    {
      ...
      if (pts.size() == 3)
          return mu0InterpolatedU(pts[0], pts[1], pts[2],
              u[0], u[1], u[2], unit_u);
      if ( pts.size() == 4 && q)
	return mu0InterpolatedU( pts[ 0 ], pts[ 1 ], pts[ 3 ], pts[ 2 ],
				 u[ 0 ], u[ 1 ], u[ 3 ], u[ 2 ] );
      const RealPoint   b = barycenter( pts );
  ...
  public:
    ...
    ( const RealPoint& a, const RealPoint& b, const RealPoint& c, const RealPoint& d,
      const RealVector& ua, const RealVector& ub, const RealVector& uc, const RealVector& ud)
    {
      const RealVector uad = ud - ua;
      const RealVector ubc = uc - ub;
      const RealVector a_cross = ubc.crossProduct(a);
      const RealVector b_cross = - uad.crossProduct(b);
      const RealVector c_cross = uad.crossProduct(c);
      const RealVector d_cross = - ubc.crossProduct(d);


      return (1/12.0) * ( ua.dot( 2 * a_cross - (uc + ud).crossProduct(b) + (ub + ud).crossProduct(c) + d_cross )
                        + ub.dot( (uc + ud).crossProduct(a) + 2 * b_cross + c_cross - (ua + uc).crossProduct(d) )
                        + uc.dot(-(ub + ud).crossProduct(a) + b_cross + 2 * c_cross + (ua + ub).crossProduct(d) )
                        + ud.dot( a_cross + (ua + uc).crossProduct(b) - (ua + ub).crossProduct(c) + 2 * d_cross ) );
    }
    ...
    static
    Scalar mu1InterpolatedU( const RealPoints& pts, const RealVectors& u, int q,
			     bool unit_u = false )
    {
      ASSERT( pts.size() == u.size() );
      if ( pts.size() <  3 ) return 0.0;
	...
	return mu1InterpolatedU( pts[ 0 ], pts[ 1 ], pts[ 2 ],
				 u[ 0 ], u[ 1 ], u[ 2 ], unit_u );
      if (pts.size() == 4 && q)
          return mu1InterpolatedU( pts[ 0 ], pts[ 1 ], pts[ 3 ], pts[ 2 ],
				 u[ 0 ], u[ 1 ], u[ 3 ], u[ 2 ] );
      const RealPoint   b = barycenter( pts );
      const RealVector ub = averageUnitVector( u );
      Scalar          a = 0.0;
  ...
  public:
    ...
    (const RealVector& ua, const RealVector& ub, const RealVector& uc, const RealVector& ud)
    {
      return (1/36.0) * ( ( 4 * ua + 2 * ub + 2 * uc +     ud ).dot( (ub - ua).crossProduct( uc - ua ) )
                        + ( 2 * ua + 4 * ub +     uc + 2 * ud ).dot( (ub - ua).crossProduct( ud - ub ) )
                        + ( 2 * ua +     ub + 4 * uc + 2 * ud ).dot( (ud - uc).crossProduct( uc - ua ) )
                        + (     ua + 2 * ub + 2 * uc + 4 * ud ).dot( (ud - uc).crossProduct( ud - ub ) ) );
    }
    ...
    static
    Scalar mu2InterpolatedU( const RealPoints& pts, const RealVectors& u, int q,
			     bool unit_u = false )
    {
      ASSERT( pts.size() == u.size() );
      if ( pts.size() <  3 ) return 0.0;
	...
	return mu2InterpolatedU( pts[ 0 ], pts[ 1 ], pts[ 2 ],
				 u[ 0 ], u[ 1 ], u[ 2 ], unit_u );
      if (pts.size() == 4 && q)
          return mu2InterpolatedU(u[ 0 ], u[ 1 ], u[ 3 ], u[ 2 ] );
      const RealPoint   b = barycenter( pts );
      const RealVector ub = averageUnitVector( u );
      Scalar          a = 0.0;
      for ( Index i = 0; i < pts.size(); i++ )
